...
Utilities for mutual fund distributors. This repository currently contains one tool:

| Folder | Description |
|--------|-------------|
| [**Rebalancer**](Rebalancer/) | Reads each client's **holdings**, applies a target allocation, and writes `output/generate_transactions.xlsx` with suggested switches. |

Setup and run instructions for Rebalancer are below. Runs on **Windows 10/11** and **macOS**. No paid software.
...
2. Unzip. You will get a folder like `MFD_Tools-main` with a `Rebalancer` folder inside it. All steps below use the **`Rebalancer`** folder as the **working directory**.
